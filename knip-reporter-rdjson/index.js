...
const ISSUE_TYPES = {
  ...
  exports: { message: 'Unused export', severity: 'ERROR' },
  nsExports: { message: 'Unused export in namespace', severity: 'ERROR' },
  types: { message: 'Unused type', severity: 'ERROR' },
  nsTypes: { message: 'Unused type in namespace', severity: 'ERROR' },
...
export default async function reporter({ issues = {}, cwd } = {}) {
  ...
  try {
    ...
    const filesIssues = Object.entries(issues._files ?? {}).flatMap(([filePath, fileIssues]) =>
      (fileIssues ?? [])
        .filter(issue => issue && ISSUE_TYPES[issue.type])
    ...
    rdjson.diagnostics.push(...filesIssues, ...typeIssues);
  } catch (error) {
    console.error('Error processing knip issues:', error.message);
  }
